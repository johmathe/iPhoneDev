...
@property(strong, nonatomic)
    SimpleVisualizationController* visualizationController;
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  self.visualizationController =
      [[SimpleVisualizationController alloc] initWithGLView:self.glview
...
- (BOOL)shouldAutorotateToInterfaceOrientation:
  ...
  // Important notice:
  // Since the logical interface orientation differs from coordinate system of
  // video frame
  // We force the interface orientation to landscape right for simplicity.
  // This orientation has one-to-one coordinates mapping from frame buffer to
  // view.
...
- (void)frameReady:(BGRAVideoFrame)frame {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
      [self.visualizationController
          setTransformationList:(self.markerDetector->getTransformations)()];
