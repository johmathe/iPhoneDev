...
- (void)updateBackground:(BGRAVideoFrame)frame {
  ...
  glBindTexture(GL_TEXTURE_2D, m_backgroundTextureId);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, frame.width, frame.height, 0, GL_BGRA,
...
- (void)setTransformationList:
            (const std::vector<Transformation> &)transformations {
  m_transformations = transformations;
}

- (void)buildProjectionMatrix:(Matrix33)
                 cameraMatrix:(int)
                 screen_width:(int)
                screen_height:(Matrix44 &)projectionMatrix {
  ...
  float f_y =
      cameraMatrix.data[4];  // Focal length in y axis (usually the same?)
...
- (void)drawBackground {
  ...
  const GLfloat squareVertices[] = {0, 0, w, 0, 0, h, w, h};

  static const GLfloat textureVertices[] = {1, 0, 1, 1, 0, 0, 0, 1};

  static const GLfloat proj[] = {0, -2.f / w, 0, 0, -2.f / h, 0, 0, 0,
                                 0, 0,        1, 0, 1,        1, 0, 1};
...
- (void)drawAR {
  ...
  [self buildProjectionMatrix:m_calibration.getIntrinsic():m_frameSize.
                        width:m_frameSize.
