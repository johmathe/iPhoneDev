...
CameraCalibration::CameraCalibration(float fx, float fy, float cx, float cy) {
  fx = 6.24860291e+02 * (640. / 352.);
  fy = 6.24860291e+02 * (480. / 288.);
  cx = 640 * 0.5f;
  cy = 480 * 0.5f;
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 3; j++) m_intrinsic.mat[i][j] = 0;
  ...
  for (int i = 0; i < 4; i++) m_distorsion.data[i] = 0;
}
...
CameraCalibration::CameraCalibration(float fx, float fy, float cx, float cy,
  ...
  for (int i = 0; i < 4; i++) m_distorsion.data[i] = distorsionCoeff[i];
}

void CameraCalibration::getMatrix34(float cparam[3][4]) const {
  for (int j = 0; j < 3; j++)
    for (int i = 0; i < 3; i++) cparam[i][j] = m_intrinsic.mat[i][j];

  for (int i = 0; i < 4; i++) cparam[3][i] = m_distorsion.data[i];
}
